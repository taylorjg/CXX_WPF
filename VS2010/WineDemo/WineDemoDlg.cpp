...
BOOL CWineDemoDlg::OnInitDialog ()
{
	...
	m_WineListCtrl.Create (NULL, NULL, WS_VISIBLE | WS_CHILD, l_rcPlaceHolder, this, IDC_WINELIST);
	...
	m_WineListCtrl.GetWindowRect (m_rcWineListCtrl);
	...
	//l_pProductQuery->AddQueryOption ("$filter", "substringof('Shiraz', Name)");

	CEntityCollection<CProduct>::type l_pProducts = l_pProductQuery->Execute ();
	m_WineListCtrl.DisplayProducts(l_pProducts);
...
void CWineDemoDlg::OnSize (UINT nType, int cx, int cy) 
{
	...
	if (m_btnOK.GetSafeHwnd () != NULL) {
		...
		m_WineListCtrl.MoveWindow (m_rcWineListCtrl, l_bRepaint);
...
//*   Description: 
//*****************************************************************************
LRESULT CWineDemoDlg::OnNcHitTest (CPoint point)
{
	LRESULT l_uResult = CDialog::OnNcHitTest (point);
