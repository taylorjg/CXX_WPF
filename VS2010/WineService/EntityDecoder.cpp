	...
	const std::vector<CComplexTypeDescriptor>&	p_ComplexTypeDescriptorTable)
{
	try {
		...
		for (	auto it = p_PropertyDescriptorTable.begin ();
				it != p_PropertyDescriptorTable.end ();
				++it)
		{
			const CPropertyDescriptor& l_PropertyDescriptor = (*it);
		...
		for (	auto it = p_ComplexTypeDescriptorTable.begin ();
				it != p_ComplexTypeDescriptorTable.end ();
				++it)
		{
			const CComplexTypeDescriptor& l_ComplexTypeDescriptor = (*it);
